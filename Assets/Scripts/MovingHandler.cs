namespace Chess
{
    ...
    public class MovingHandler : MonoBehaviour
    {
        ...
        static bool upgraded, castled, kingSide, enPassant;
        ...
        public static Move MakeMove(Piece selectedPiece, int[] moveTo, bool updateGraphics = true)
        {
            ...
            if (selectedPiece != null && moveTo != null && !Board.capturing)
            {
                ...
                move = new Move(selected, moveFrom, moveTo, moved, false, null, upgraded, upgraded ? selectedPiece : null, enPassant, enPassantSquare, enPassantPiece, castled, kingSide, Board.castlingRook, pieces);
            }
            else if (selectedPiece != null && moveTo != null && Board.capturing)
            {
                ...
                Essentials.ChangeTurn();
                move = new Move(selected, moveFrom, moveTo, moved, Board.capturing, capturedPiece, upgraded, upgraded ? selectedPiece : null, false, enPassantSquare, enPassantPiece, false, false, null, pieces);
        ...
        static void CheckKingCastling(Piece selectedPiece, int[] moveTo, bool updateGraphics)
        {
            ...
            {
                kingSide = moveTo[0] - selectedPiece.position[0] == 2;
                Board.castlingRook = Board.pieces.FirstOrDefault(x => x.position.SequenceEqual(new int[2] { (kingSide ? 7 : 0), (Essentials.CheckColor(selectedPiece, ChessPieceTypes.White) ? 0 : 7) }));

                if (Board.castlingRook != null)
                {
                    int[] castlingSquare = new int[2] { selectedPiece.position[0] + (kingSide ? 1 : -1), selectedPiece.position[1] };
